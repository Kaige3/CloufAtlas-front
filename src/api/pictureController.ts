// @ts-ignore
/* eslint-disable */
import request from "@/utils/request";

/** deletePicture POST /api/delete */
export async function deletePictureUsingPost(
  body: API.DeleteRequest,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseBoolean_>("/api/delete", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    data: body,
    ...(options || {}),
  });
}

/** editPicture POST /api/edit */
export async function editPictureUsingPost(
  body: API.PictureEditDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseBoolean_>("/api/edit", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    data: body,
    ...(options || {}),
  });
}

/** batchEditePicture POST /api/edit/batch */
export async function batchEditePictureUsingPost(
  body: API.BatchEditePictureDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseBoolean_>("/api/edit/batch", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    data: body,
    ...(options || {}),
  });
}

/** getPictureById GET /api/get */
export async function getPictureByIdUsingGet(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.getPictureByIdUsingGETParams,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponsePicture_>("/api/get", {
    method: "GET",
    params: {
      ...params,
    },
    ...(options || {}),
  });
}

/** getPictureVOById GET /api/get/vo */
export async function getPictureVoByIdUsingGet(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.getPictureVOByIdUsingGETParams,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponsePictureVO_>("/api/get/vo", {
    method: "GET",
    params: {
      ...params,
    },
    ...(options || {}),
  });
}

/** listPictureByPage POST /api/list/page */
export async function listPictureByPageUsingPost(
  body: API.PictureQueryDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponsePagePicture_>("/api/list/page", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    data: body,
    ...(options || {}),
  });
}

/** listPictureVOByPage POST /api/list/page/vo */
export async function listPictureVoByPageUsingPost(
  body: API.PictureQueryDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponsePagePictureVO_>("/api/list/page/vo", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    data: body,
    ...(options || {}),
  });
}

/** listPictureVoByPageWithCache POST /api/list/page/vo/cache */
export async function listPictureVoByPageWithCacheUsingPost(
  body: API.PictureQueryDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponsePagePictureVO_>("/api/list/page/vo/cache", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    data: body,
    ...(options || {}),
  });
}

/** listPictureVoByPageWithCaffeineCache POST /api/list/page/vo/Caffeinecache2 */
export async function listPictureVoByPageWithCaffeineCacheUsingPost(
  body: API.PictureQueryDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponsePagePictureVO_>(
    "/api/list/page/vo/Caffeinecache2",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      data: body,
      ...(options || {}),
    }
  );
}

/** listPictureVoByPageWithCaffeineCacheAndRedis POST /api/list/page/vo/Caffeinecache3 */
export async function listPictureVoByPageWithCaffeineCacheAndRedisUsingPost(
  body: API.PictureQueryDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponsePagePictureVO_>(
    "/api/list/page/vo/Caffeinecache3",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      data: body,
      ...(options || {}),
    }
  );
}

/** createOutPaintingTask POST /api/out_painting/create_task */
export async function createOutPaintingTaskUsingPost(
  body: API.CreatePictureOutPaintingTaskRequest,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseCreateOutPaintingTaskResponse_>(
    "/api/out_painting/create_task",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      data: body,
      ...(options || {}),
    }
  );
}

/** GetOutPaintingTask GET /api/out_painting/get_task */
export async function getOutPaintingTaskUsingGet(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.GetOutPaintingTaskUsingGETParams,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseGetOutPaintingTaskResponse_>(
    "/api/out_painting/get_task",
    {
      method: "GET",
      params: {
        ...params,
      },
      ...(options || {}),
    }
  );
}

/** doPictureReview POST /api/review */
export async function doPictureReviewUsingPost(
  body: API.PictureReviewDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseBoolean_>("/api/review", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    data: body,
    ...(options || {}),
  });
}

/** searchPictureByColor POST /api/search/color */
export async function searchPictureByColorUsingPost(
  body: API.SearchPictureByColorDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseListPictureVO_>("/api/search/color", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    data: body,
    ...(options || {}),
  });
}

/** searchPicture POST /api/search/picture */
export async function searchPictureUsingPost(
  body: API.SearchPictureByPictureDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseListSoImageSearchDto_>("/api/search/picture", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    data: body,
    ...(options || {}),
  });
}

/** getTagAndCategory GET /api/tag_category */
export async function getTagAndCategoryUsingGet(options?: {
  [key: string]: any;
}) {
  return request<API.BaseResponsePictureTagCategory_>("/api/tag_category", {
    method: "GET",
    ...(options || {}),
  });
}

/** updatePicture POST /api/update */
export async function updatePictureUsingPost(
  body: API.PictureUpdateDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseBoolean_>("/api/update", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    data: body,
    ...(options || {}),
  });
}

/** uploadPicture POST /api/upload */
export async function uploadPictureUsingPost(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.uploadPictureUsingPOSTParams,
  body: {},
  file?: File,
  options?: { [key: string]: any }
) {
  const formData = new FormData();

  if (file) {
    formData.append("file", file);
  }

  Object.keys(body).forEach((ele) => {
    const item = (body as any)[ele];

    if (item !== undefined && item !== null) {
      if (typeof item === "object" && !(item instanceof File)) {
        if (item instanceof Array) {
          item.forEach((f) => formData.append(ele, f || ""));
        } else {
          formData.append(ele, JSON.stringify(item));
        }
      } else {
        formData.append(ele, item);
      }
    }
  });

  return request<API.BaseResponsePictureVO_>("/api/upload", {
    method: "POST",
    params: {
      ...params,
    },
    data: formData,
    requestType: "form",
    ...(options || {}),
  });
}

/** uploadPictureByBatch POST /api/upload/batch */
export async function uploadPictureByBatchUsingPost(
  body: API.PictureUploadByBatchDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseInt_>("/api/upload/batch", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    data: body,
    ...(options || {}),
  });
}

/** uploadPictureByUrl POST /api/upload/url */
export async function uploadPictureByUrlUsingPost(
  body: API.PictureUploadDto,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponsePictureVO_>("/api/upload/url", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    data: body,
    ...(options || {}),
  });
}
